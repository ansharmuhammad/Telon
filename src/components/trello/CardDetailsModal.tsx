...
import { Card as CardType, List as ListType, Label as LabelType, CoverConfig, Checklist as ChecklistType, ChecklistItem, Attachment as AttachmentType } from '@/types/trello';
...
import { CalendarIcon, Trash2, Link2, Image as CoverIcon, Paperclip, MoreVertical, Download, Edit } from 'lucide-react';
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 pl-9">
                          {card.attachments.map(attachment => (
                            <div key={attachment.id} className="flex items-center gap-4 group">
                              <a href={getPublicUrl('card-attachments', attachment.file_path)} target="_blank" rel="noopener noreferrer" className="w-16 h-12 bg-gray-200 rounded flex items-center justify-center font-bold text-gray-500 text-sm hover:bg-gray-300">
                                {attachment.file_type?.split('/')[1]?.toUpperCase().substring(0, 4) || 'FILE'}
                              </a>
                              <div className="flex-grow">
                                <a href={getPublicUrl('card-attachments', attachment.file_path)} target="_blank" rel="noopener noreferrer" className="font-medium hover:underline">{attachment.file_name}</a>
                                <p className="text-xs text-muted-foreground">Added {formatDistanceToNow(new Date(attachment.created_at), { addSuffix: true })}</p>
                              </div>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                  ...
                                  <DropdownMenuItem onClick={() => { setRenamingAttachment(attachment); setNewAttachmentName(attachment.file_name); }}>
                                    <Edit className="mr-2 h-4 w-4" /> Rename
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => onDeleteAttachment(attachment.id)} className="text-red-500 focus:text-red-500">
                  ...
                  <div className="space-y-2">
                    ...
                    <AttachmentPopover onAddAttachment={(file) => onAddAttachment(card.id, file)}>
